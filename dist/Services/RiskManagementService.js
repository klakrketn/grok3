...
class RiskManagementService {
    constructor(marketAnalysis, monitoring, logger, maxDailyTrades = 50) {
        this.marketAnalysis = marketAnalysis;
        this.monitoring = monitoring;
        this.logger = logger;
        this.maxDailyTrades = maxDailyTrades;
        this.MAX_EXPOSURE_TIME = 24 * 60 * 60 * 1000; // 24 часа
        this.RISK_CHECK_INTERVAL = 5 * 60 * 1000; // 5 минут
    ...
    startRiskMonitoring() {
        this.checkInterval = setInterval(() => this.checkAllPositions(), this.RISK_CHECK_INTERVAL);
    }
    stopRiskMonitoring() {
    ...
    async evaluateTradeRisk(mint, amount, price, portfolioValue, riskProfile = 'default') {
        try {
    ...
    async calculateRiskScore(metrics, amount, price, portfolioValue) {
        ...
        return Object.entries(weights)
            .reduce((score, [key, weight]) => score + (scores[key] * weight), 0);
    }
    async calculateVolatility(mint) {
        const riskMetrics = await this.marketAnalysis.calculateRiskMetrics(mint);
        return riskMetrics?.volatility || 1;
    }
    calculateStopLoss(price, level) {
        return price * (1 - level / 100);
    }
    calculateTakeProfit(price, level) {
        return price * (1 + level / 100);
    }
    countOpenPositions() {
        return this.positionRisks.size;
    }
    async updatePosition(mint, amount, price, type) {
    ...
    async calculatePositionRisk(mint, amount, price) {
        ...
        if (!riskMetrics)
            return 1;
        return ((riskMetrics.volatility * 0.3) +
            (riskMetrics.maxDrawdown * 0.3) +
            ((1 - riskMetrics.successRate) * 0.4));
    }
    async updateDailyStats(mint) {
        const trades = this.dailyStats.trades.get(mint) || [];
        if (trades.length < 2)
            return;
    ...
    async checkAllPositions() {
        try {
            for (const [mint, position] of this.positionRisks.entries()) {
                ...
                const riskScore = await this.calculatePositionRisk(mint, position.exposure / currentPrice, currentPrice);
